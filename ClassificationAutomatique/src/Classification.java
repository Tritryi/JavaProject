    ...
    public static void  classementDepeche(ArrayList<Depeche> depeches, ArrayList<Categorie> categories, String nomFichier){
        try {
            ...
            for (Depeche depeche : depeches) {
                ...
                String cm = UtilitairePaireChaineEntier.chaineMax(scores);
                writer.write(depeche.getId() + ":" + UtilitairePaireChaineEntier.chaineMax(scores) +"\n");

                ArrayList<PaireChaineEntier> tab_rep = new ArrayList<>();
                PaireChaineEntier pa = new PaireChaineEntier(depeche.getCategorie(),1);
                if (depeche.getCategorie().equals(cm)){
                    tab_rep.add(pa);
                }
                writer.write();
