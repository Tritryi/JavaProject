    ...
    public static void  classementDepeche(ArrayList<Depeche> depeches, ArrayList<Categorie> categories, String nomFichier){
        ...
        try {
            ...
            for(int i=0; i<depeches.size();i++){
                ...
                ArrayList<PaireChaineEntier> p = new ArrayList<>();
                for(Categorie c : categories){
                    p.add(new PaireChaineEntier(c.getNom(),c.score(depeches.get(i))));
                }
                file.write(UtilitairePaireChaineEntier.chaineMax(p));
            }
            file.close();
